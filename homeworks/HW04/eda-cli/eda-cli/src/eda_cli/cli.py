...
def report(
    ...
    min_missing_share: float = typer.Option(0.0, help="Порог доли пропусков для проблемных колонок."),
    ...
    top_cats = top_categories(df, max_columns=max_hist_columns, top_k=top_k_categories) # заменил
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Подозрительные ID: {quality_flags['suspicious_id_columns']}\n\n")

        f.write("## Колонки\n\n")
        f.write("См. файл `summary.csv`.\n\n")

        f.write("## Пропуски\n\n")
        # Фильтруем проблемные колонки по min_missing_share
        ...
        if not filtered_missing.empty:
            ...
            for col in filtered_missing.index:
                f.write(f"- {col}: {filtered_missing.loc[col,'missing_share']:.2%}\n")
    ...
    plot_missing_matrix(df, out_root / "missing_matrix.png", min_missing_share=min_missing_share) # Новая визуализация
