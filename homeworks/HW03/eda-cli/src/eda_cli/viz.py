...
def plot_missing_matrix(df: pd.DataFrame, out_path: PathLike, min_missing_share: float = 0.0) -> Path:
    if min_missing_share > 0:
        cols_to_plot = df.columns[df.isna().mean() >= min_missing_share]
    ...
    if df_to_plot.empty:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, "No missing values above threshold", ha="center", va="center")
        ax.axis("off")
    else:
        mask = df_to_plot.isna().values
        fig, ax = plt.subplots(figsize=(min(12, df_to_plot.shape[1] * 0.4), 4))
        ax.imshow(mask, aspect="auto", interpolation="none")
        ax.set_xticks(range(df_to_plot.shape[1]))
        ax.set_xticklabels(df_to_plot.columns, rotation=90, fontsize=8)
        ax.set_yticks([])
        ax.set_title(f"Missing values matrix (>= {min_missing_share:.0%})")
